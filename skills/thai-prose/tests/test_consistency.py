    ...
    for f in SKILL_FILES:
        for line_no, line in enumerate(f.read_text("utf-8").splitlines(), start=1):
            ...
            for m in SLUG_REF_RE.finditer(line):
                ref = m.group(1)
                # `fN/sub-slug` resolves via its umbrella frame `fN` (the frames are
                # defined in SKILL.md; sub-slugs are finer pointers under them).
                # `rule-NN-*` refs still require an explicit `###` definition.
                if ref in defined or ref.split("/", 1)[0] in FRAME_UMBRELLA:
                    continue
                missing.append(f"{f.name}:{line_no}: `{ref}`")

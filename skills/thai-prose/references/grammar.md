...
Word- and character-level Thai grammar — hard rules. This file is a **union** of two
sources:

- **Universal base** (kien-thai, MIT) — word/phrase-level rules that hold across *all
  registers*, tagged `*(… · all-registers · …)*`.
- **Scaler domain** (thai-content) — the same rules specialized to the
  *scaler peer-explainer* register, plus the mechanical linter-enforced rules, tagged
  `*(… · scaler-explainer · …)*`.

Distinct from the discourse frames in `SKILL.md`. The first two entries below are
**mechanical and linter-enforced**: `scripts/lint_thai_content.py` emits their exact
slugs (`rule-07-period`, `rule-08-yamok`), and SKILL routing maps a mechanical lint hit
to its keyed reference entry here. The rest are judgment/surface rules.

**On universal-vs-scaler collisions:** where a universal rule and a scaler rule disagree
(e.g. ไม้ยมก spacing), both sides are preserved with their tags. This file does **not**
resolve the conflict — `overrides.md` is the authority, and SCALER overrides UNIVERSAL
in scaler-explainer content.

---

## Mechanical / linter-enforced rules (scaler-explainer)
...
> Provenance: this is the word-level enforcement of the universal Frame 3 (sentence
> boundaries via space and paragraph, not period — see `SKILL.md` `f3`, which covers
...
> **Collision (preserved, not resolved):** the universal register keeps ไม้ยมก *spaced*
> (`ต่าง ๆ` always spaced); the scaler register *never* spaces before `ๆ`. `overrides.md`
> is the authority (SCALER > UNIVERSAL). See the universal side under `mai-yamok-spacing`
> below.

### `mai-yamok-spacing` *(mechanical · all-registers · hard)*

**Universal side of the ไม้ยมก collision.** In the universal/kien-thai register, ไม้ยมก
(`ๆ`) is written *with* a space before it — e.g. `ต่าง ๆ` stays spaced. This is the
opposite of the scaler override in `rule-08-yamok` above. Both are intentionally kept;
`overrides.md` decides which applies for a given piece of content (SCALER > UNIVERSAL in
scaler-explainer content).

---

## Word-level rules

### `wrong-classifier` *(mechanical · all-registers · hard)* / *(judgment · scaler-explainer · hard)*
...
Common technical-noun classifiers (universal list):
...
**Scaler-explainer specialization** *(domain · scaler-explainer · hard)*: same rule,
with the scaler-tuned classifier list and examples —
...
### `missing-cha-modal` *(mechanical · all-registers · hard)* / *(judgment · scaler-explainer · hard)*
...
### `verb-calque` *(mechanical · all-registers · hard)* / *(judgment · scaler-explainer · hard)*
...
**Scaler-explainer specialization** *(domain · scaler-explainer · hard)*: same rule,
watch idiomatic-physical verbs (burst, drop, spin up, throttle) —

- **Bad**: `ถ้า queue เต็มก็ทิ้ง` (calque of "drop")
- **Good**: `ถ้า queue เต็มก็โดน reject`

### `capability-modal` *(mechanical · all-registers · hard)* / *(judgment · scaler-explainer · soft)*
...
**Scaler-explainer specialization** *(domain · scaler-explainer · soft)*: in this
register `V + ได้` alone is fine; reach for the full frame only for emphasis.

- **Acceptable**: `ระบบ query ได้ผ่าน API`
- **Emphatic**: `ระบบสามารถ query ได้ผ่าน API`

### `time-period` *(mechanical · all-registers · hard)* / *(judgment · scaler-explainer · hard)*

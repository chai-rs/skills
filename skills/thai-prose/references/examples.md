...
ตอน self-edit หรือตอนตรวจงานเขียนภาษาไทย ตัวอย่างเหล่านี้ register-tagged —
scope ไปที่ register ที่กำลังเขียนอยู่ แล้วเทียบเฉพาะกลุ่มนั้น

ข้อความไทยยาว ๆ ใน blockquote ถือเป็นชิ้นเดียวที่แตะไม่ได้ ความยาวเกิน 90 column
ก็ปล่อยให้เกินไป

Format ของแต่ละตัวอย่าง: ฉบับ AI-shaped **Before**, **Tells** ที่มันสะดุด (อ้างด้วย
slug ของ rule), และ **After** ที่แก้แล้ว ตัวอย่างกลุ่ม "Scaler peer-explainer"
ผูกกับ reference voice ของ scaler — *what-is-math* ของ LearnAlgorithm (โครงไทย +
ศัพท์อังกฤษ, problem-first, particle ธรรมชาติ) ส่วน anti-voice คือโทนบล็อกของ
DataRockie (แอด/555/emoji) ที่ house voice แบนไว้ กลุ่มนี้ map เข้ากับ Explainer
register

> Note: กลุ่ม scaler ยังไม่มี curated native-Thai corpus ตัวอย่างเป็นภาพประกอบ ไม่ใช่
> corpus exemplar ค่อย seed ของจริงจากบทเรียนที่ผ่าน linter อย่าหยิบ output บทเรียนที่
> Claude เขียนมาใช้เป็น native anchor
...
<!-- register: explainer (scaler peer-explainer) -->
## Scaler peer-explainer

ตัวอย่างกลุ่มนี้อยู่ใน register รุ่นพี่เล่าให้รุ่นน้องฟัง (scaler house voice)
map เข้ากับ Explainer register แต่มีโทนเฉพาะของ scaler — problem-first,
ทับศัพท์ที่ noun, particle ธรรมชาติ, ไม่มีแอด/555/emoji

### Scaler Example 1 — bureaucratic open → problem-first
...
### Scaler Example 2 — period spam + dangling clauses → space + ก็
...
### Scaler Example 3 — calqued connectives → native pivot
...
### Scaler Example 4 — wrong code-mix → mix at the noun

# AI tells in Thai prose

Patterns that read as machine-written, unified into one reference. Two layers live here:

- **Universal base (mechanical)** — Thai-correctness rules that apply across all registers
  (marketing, technical, narrative, conversational). Cross-references to Frames F1–F7 in
  SKILL.md. Register-conditional rules tagged `· register`. Companions: `craft.md` (soft),
  `grammar.md` (surface). Inline-code Thai examples may exceed the 90-column rule.
- **Domain layer (scaler peer-explainer)** — rules that specialize the house register
  (รุ่นพี่ที่เคยผ่านมาแล้ว เล่าให้ฟังตรง). Migrated from CONTENT-GUIDE §9 + kien-thai's
  ai-tells and the surviving craft rules (MIT). Most are judgment-tier; the grep-able phrase
  tells live in `forbidden-phrases.md` (linter).

Tags tell the two layers apart: universal rules carry `*(mechanical · all-registers · ...)*`
(or `· register`); scaler-domain rules carry `*(... · scaler-explainer · ...)*`. This file
does **not** resolve universal-vs-scaler collisions — `overrides.md` is the authority
(SCALER > UNIVERSAL). Both layers are preserved here with their tags so readers can tell
them apart.
...
Note (Thai abbreviations): the period also lands legitimately inside standard Thai
abbreviation forms — time units (`น.`, `ชม.`), month abbreviations (`ม.ค.`, `ก.พ.`,
`ธ.ค.`), and titles (`ดร.`). These are not mid-paragraph-period violations; any
linter check for "Thai char followed by period" must allowlist them.
...
## Scaler peer-explainer layer

These rules specialize the house register. Most are judgment-tier; the grep-able phrase
tells they reference live in `forbidden-phrases.md` (linter).
...
**Empty intensifiers** (canonical rule: `empty-intensifier` in `craft.md`). Intensifiers
that add no information: `อย่างมาก`, `เป็นอย่างยิ่ง`, `อย่างมหาศาล`. Cut, or replace with a
concrete number.
...
*Universal base rules are extended by the scaler peer-explainer layer. Universal-vs-scaler
collisions are resolved in `overrides.md` (SCALER > UNIVERSAL).*
